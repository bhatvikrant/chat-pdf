...
import { type DrizzleChat, chats } from "~/server/db/schema";
import ChatSidebar from "./Chat/ChatSidebar";

const Navbar = async () => {
  const { userId } = auth();

  let chatsData: DrizzleChat[];

  if (userId) {
    chatsData = await db.select().from(chats).where(eq(chats.userId, userId));
  }
  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="flex h-14 max-w-screen-2xl items-center px-4 sm:container">
          <div className="mr-4 flex">
            <a
              className={cn(`group mr-6 flex items-center space-x-2`, {
                "ml-16": !!userId,
              })}
              href="/"
            >
              ...
              <span className="inline-block font-bold">ChatPDF</span>
            </a>
          </div>

          <div className="flex flex-1 items-center justify-between sm:space-x-2 md:justify-end">
      ...
      {userId && chatsData! && <ChatSidebar chats={chatsData} chatId="" />}
    </>
  );
};
